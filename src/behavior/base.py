...
class RefinementActor(nn.Module):
    def __init__(self,
        ...
        # TODO: Should improve this, not ideal!
    ...
    @torch.no_grad()
    def action_pred(self, batch):
        nobs = self.bc_actor._training_obs(batch, flatten=self.flatten_obs)
        naction = self.bc_actor._normalized_action(nobs)
    ...
    def base_action_normalized(self, obs: Dict[str, torch.Tensor]) -> torch.Tensor:
        action = self.bc_actor.action(obs)
        return self.normalizer(action, 'action', forward=True)
    ...
    def get_value(self, residual_nobs) -> Tensor:
    ...
    def reset(self):
        ...
        self.bc_actor.observations.clear()
        if self.bc_actor.actions is not None:
            self.bc_actor.actions.clear()

    def correct_action(self, obs: Dict[str, Tensor], action: Tensor) -> Tensor:
        nobs = self.process_obs(obs)
        naction = self.normalizer(action, 'action', forward=True)

        residual_nobs = torch.cat([nobs, action], dim=-1)

        naction_corrected = (
            naction + self.residual_policy.actor_mean(residual_nobs)
            * self.residual_policy.action_scale
        )

        # Return the un-normalized, corrected action
    ...
    def base_actor_parameters(self):
        ...
        return [
            p
            for n, p in self.bc_actor.model.named_parameters()
            # TODO: make this more general: encoder{k}.*
            if not (n.startswith("encoder1.") or n.startswith("encoder2."))
        ]
